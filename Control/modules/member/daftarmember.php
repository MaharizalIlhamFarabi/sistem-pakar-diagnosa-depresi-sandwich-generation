...
if(isset($_POST["edit"])){
    ...
    if($tgl <= $maxdate)
    {
        $sqlm = mysqli_query($con, "UPDATE member SET nama='$_POST[nama]', jk='$_POST[jk]', alamat='$alamat', tgl_lahir='$tgl', username='$_POST[username]', password='$_POST[password]'  WHERE id_member='$_POST[id_member]'");
...
if(isset($_POST["simpan"])){
    ...
    if($tgl <= $maxdate)
    {
        $sqlm = mysqli_query($con, "INSERT INTO member (nama, jk, alamat, tgl_lahir, username, password) VALUES ('$_POST[nama]', '$_POST[jk]', '$alamat',  '$tgl', '$_POST[username]', '$_POST[password]')");
...
<tr id="thpo">
 ...
 <tbody class="">
   ...
   <?php
     ...
     $sqlm = mysqli_query($con, "SELECT id_member, nama, jk, alamat, tgl_lahir,  username, password, YEAR(curdate()) - YEAR(tgl_lahir) AS umur FROM member LIMIT $mulai, $halaman") or die(mysqli_error($con));
     $no = $mulai + 1;

     while ($rm = mysqli_fetch_assoc($sqlm)) {
     ...
     <a href='?r=daftarmemberdel&idm=$rm[id_member]'class='btn btn-danger'>  <span class='glyphicon glyphicon-trash'></span>Hapus</a>
     </td>
       </tr>";
         $no++;
     }
